...
test('travel do not have tours', function () {
    $travel = Travel::factory()->create();
...
test('travel have tours', function () {
...
test('unauthenticated user can not see delete tour link', function () {
    $this->get('/travels/'.$this->travel->slug)->assertStatus(200)->assertDontSee('delete-travel-form');
});

test('unauthenticated user can not see edit tour link', function () {
    $this->get('/travels/'.$this->travel->slug)->assertStatus(200)->assertDontSee('edit-travel-link');
});
...
test('editor user can not see add tour and delete tour links', function () {
    $this->actingAs($this->editorUser)
        ->get('/travels/'.$this->travel->slug)
        ->assertStatus(200)->assertDontSee('add-tour-link')->assertDontSee('delete-tour-form');
});
