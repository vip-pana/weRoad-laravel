...
test('dashboard have a second page with a travel', function () {
    ...
    $travel = $travels->last();
    $this->get('/?page=2')->assertStatus(200)
        ->assertViewHas('travels', function ($collection) use ($travel) {
            return $collection->contains($travel);
        });
});
...
test('admin user can see logout form and can not see login link', function () {
    $adminUser = User::factory()->create(['role' => UserRole::ADMIN]);
    $this->actingAs($adminUser)->get('/')
        ->assertStatus(200)->assertSee('logout-form')->assertDontSee('login-link');
});
