...
test('admin user can access tours.create', function () {
    $this->actingAs($this->adminUser)->get('/travels/'.$this->travel->slug.'/tours/create')->assertStatus(200);
});
...
test('admin user can access tours.edit', function () {
    $tour = $this->travel->tours[0];

    $this->actingAs($this->adminUser)->get('/tours/'.$tour->id.'/edit/')->assertStatus(200);
});
